...
def _make_score_matrix_generic(
    ...
    print('Is Global' if align_globally else 'Is Local')
    for i in range(lenA + 1):
        if align_globally and penalize_end_gaps[1]:  # [1]:gap in sequence B
            score = gap_B_fn(0, i)
        else:
            score = 0
        score_matrix[i][0] = score
    for i in range(lenB + 1):
        if align_globally and penalize_end_gaps[0]:  # [0]:gap in sequence A
            score = gap_A_fn(0, i)
        else:
            score = 0
